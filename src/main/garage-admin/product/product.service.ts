...
export class ProductService {
    ...
    async create(
        ...
        const { sellerEmail, sellerName, sellerPhoneNumber, sellerType, sellerIsVerified, photos, ...productData } = createProductDto;
    ...
    async update(
        ...
        const { sellerName, sellerEmail, sellerPhoneNumber, sellerType, sellerIsVerified, photos, ...productData } = updateProductDto;
        ...
        if (sellerName || sellerEmail || sellerPhoneNumber || sellerType || sellerIsVerified !== undefined) {
            ...
            if (sellerIsVerified !== undefined) sellerUpdateData.isVerified = sellerIsVerified;
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (photoUrls.length > 0) {
                ...
                await Promise.all(
                    ...
                    photosToDelete.map(photo => (this.s3FileService as any).deleteFile(photo.url))
